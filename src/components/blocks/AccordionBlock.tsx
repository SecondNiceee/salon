...
export const AccordionBlock: React.FC<AccordionBlockProps> = ({ title, content }) => {
  ...
  return (
    <div className="accordion-block border border-gray-200 rounded-lg overflow-hidden my-4">
      <div className="accordion-header flex items-center justify-between p-4 bg-gray-50">
        <div className="accordion-title flex-1">
          <RichText converters={jsxConverters} data={title} />
        </div>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="accordion-button ml-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200 whitespace-nowrap"
        >
